            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {[1, 2, 3].map(n => (
                <button
                  ...
                  onClick={() => setJumlah(n)}
                  style={{
                    width: '40px', height: '40px', border: `1px solid ${jumlah === n ? '#111' : '#e0e0e0'}`,
                    borderRadius: '8px', background: jumlah === n ? '#111' : '#fff',
                    color: jumlah === n ? '#fff' : '#555', fontSize: '14px',
                    cursor: 'pointer', fontWeight: jumlah === n ? 500 : 400
                  }}
                >
                  {n}
                </button>
